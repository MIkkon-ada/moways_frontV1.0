...
const TYPE_LABEL: Record<ConfirmationType, string> = {
  progress_update: '子任务进展更新',
  subtask_completion: '子任务完成提交',
  suggest_new_subtask: '建议新增子任务',
...
export function buildConfirmationEffects(result: AnyRecord | null | undefined): string[] {
  ...
  if (isSubtaskStatusUpdate(data)) {
    const title = text(data.subtask_title) || '子任务'
    const toStatus = text(data.to_status) || text(data.suggested_status) || '目标状态'
    return [`将「${title}」状态变更为「${toStatus}」`, '保留所属关键任务状态，由负责人判断是否关闭']
  }
  ...
  if (completedCount > 0) {
    effects.push(`将 ${completedCount} 个已匹配子任务标记为已完成`)
  } else if (matchedCount > 0) {
    effects.push(`更新 ${matchedCount} 个已匹配子任务的进展记录`)
  }
  if (suggestCount > 0) effects.push(`建议新增 ${suggestCount} 个子任务，需负责人选择归属关键任务后确认`)
  if (newTaskCount > 0) effects.push(`创建 ${newTaskCount} 个新子任务草稿/任务项`)
  if (hasLegacyNewTask(data) && matchedCount === 0 && newTaskCount === 0) effects.push('写入工作推进表关键任务')
  if (achievementCount > 0) effects.push(`写入成果库 ${achievementCount} 条`)
  if (issueCount > 0) effects.push(`写入问题与决策 ${issueCount} 条`)
  if (matchedCount > 0) effects.push('保留所属关键任务状态，由负责人判断是否关闭')
...
export function getConfirmationContext(result: AnyRecord | null | undefined): ConfirmationContext {
  ...
  return {
    ...
    projectName: text(data.special_project) || text(task.special_project),
