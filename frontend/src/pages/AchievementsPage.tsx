...
export function AchievementsPage() {
  ...
  function resolveSelectedProjectId(): number | null {
    ...
    if (selected.project_id != null) return selected.project_id
    return projects.find((p) => p.name === selected.special_project)?.id ?? null
  }

  function canReviewSubmission(sub: AchievementSubmissionItem): boolean {
    if (currentUser?.is_tech_admin) return true
    const projName = sub.special_project ?? ''
  ...
  function openTaskPickerFor(item: AchievementItem) {
    const projId = item.project_id != null
      ? item.project_id
      : projects.find((p) => p.name === item.special_project)?.id ?? null
    if (!projId) { alert('该成果未关联专项，无法查找关键任务'); return }
    setSelected(item)
    setTaskLinkErr('')
  ...
  async function handleLinkTask(taskId: number | null) {
    ...
    try {
      const updated = await updateAchievement(selected.id, {
        ...
        special_project: selected.special_project ?? '',
  ...
  async function handleSubmitForReview() {
    ...
    if (!createRelatedTaskId) { setCreateError('请选择关联关键任务'); return }
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 overflow-hidden">
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <label className="space-y-1.5">
                  <span className="text-xs font-semibold text-slate-600">关联关键任务 <span className="text-red-400">*</span></span>
                  <select
                  ...
                  >
                    <option value="">
                      {!createProjectId ? '请先选择专项' : createFormTasksLoading ? '加载中…' : createFormTasks.length === 0 ? '该专项暂无任务' : '请选择关键任务'}
                    </option>
            ...
            <div className="px-6 py-4 border-t flex items-center justify-between gap-3" style={{ borderColor: '#E9EFF6' }}>
              ...
              <div className="flex items-center gap-2">
                ...
                <button type="button" onClick={handleSubmitForReview} disabled={createSaving} className="px-4 py-2 rounded-lg text-white text-sm font-semibold hover:opacity-90 disabled:opacity-50" style={{ background: 'linear-gradient(135deg,#0369A1,#0EA5E9)' }}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(15,23,42,0.45)' }}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
            ...
            <div className="px-6 py-4 border-t flex justify-end gap-2" style={{ borderColor: '#E9EFF6' }}>
              ...
              <button type="button" onClick={handleRejectSubmission} disabled={subActionLoading} className="px-4 py-2 rounded-lg text-white text-sm font-semibold hover:opacity-90 disabled:opacity-50" style={{ background: 'linear-gradient(135deg,#DC2626,#EF4444)' }}>
      ...
      {taskPickerOpen && selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(15,23,42,0.45)' }}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: '#E9EFF6' }}>
              <div>
                <h3 className="text-base font-bold text-slate-800">关联关键任务</h3>
                <p className="text-xs text-slate-400 mt-0.5">
                  {selected.special_project ? `显示「${selected.special_project}」下的关键任务` : '显示当前项目的关键任务'}
                </p>
              </div>
            ...
            <div className="max-h-80 overflow-y-auto px-6 py-4">
              {taskPickerLoading ? (
                ...
                <div className="text-center text-slate-400 text-sm py-6">该项目下暂无关键任务</div>
              ) : (
                        ...
                        <div className="flex items-start gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-3 mt-1 text-xs text-slate-400 flex-wrap">
                              {sub.special_project && <span>{sub.special_project}</span>}
          ...
          {selectedSubmission && (
            <div style={{ width: 340, flexShrink: 0, borderLeft: '1px solid #E9EFF6', background: '#fff', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
              ...
              <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
                ...
                {[
                  { label: '所属专项', value: selectedSubmission.special_project },
