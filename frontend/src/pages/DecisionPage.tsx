...
export function DecisionPage() {
  ...
  }, [])  // CEO 决策中心跨全部项目，不依赖 currentProjectId
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-4" style={{ background: '#F1F5F9' }}>
        ...
        <div className="text-center">
          ...
          <p className="text-slate-400 text-sm mt-1">CEO 决策中心仅限 CEO 角色访问</p>
        </div>
      </div>
    )
  }
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      <header className="h-16 flex items-center px-6 gap-3 flex-shrink-0 bg-white border-b" style={{ borderColor: '#E9EFF6' }}>
        <div className="flex-1">
          <h1 className="text-base font-bold text-slate-800">CEO 决策中心</h1>
          <p className="text-xs text-slate-400 mt-0.5">处理上报给 CEO 的重大决策事项</p>
        </div>
      </header>
          ...
          {selected ? (
            <>
              ...
              <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1.5">CEO 决策意见（必填）</label>
                  <textarea
                    value={note}
