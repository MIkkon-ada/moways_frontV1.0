...
| `/api/confirmations/{id}/escalate-ceo` | POST | 升级 CEO 决策 |
| `/api/confirmations/{id}/ceo-decide` | POST | CEO 批示 |
...
- 核心角色说明：
  - `super_admin`：全局管理权限
  - `owner`：项目负责人，允许处理确认中心核心动作
  - `coordinator`：统筹人，处理转交给统筹人的事项
  - `member`：普通成员，主要提交进展和查看项目数据
  - `project_ceo`：项目 CEO 决策人，处理升级决策事项
