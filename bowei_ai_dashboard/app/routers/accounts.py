...
from ..auth import hash_password, invalidate_user_sessions, _check_password
...
def _require_admin(current_user: str, db: Session):
    ...
    if not ctx["is_tech_admin"]:
        raise HTTPException(403, "仅技术管理员可管理账号")
...
def create_account(
    ...
    db.add(row)
    db.flush()
    if person and payload.is_tech_admin:
...
def reset_password(
    ...
    row.status = "active"
    row.last_password_changed_at = datetime.utcnow()
