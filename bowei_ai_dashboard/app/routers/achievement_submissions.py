...
def _require_owner_or_admin(context: dict, row: models.AchievementSubmission, db: Session) -> None:
    ...
    proj_id = row.project_id or resolve_project_id(row.special_project or "", None, db)
...
def create_submission(
    ...
    proj_name = crud.get_project_name_by_id(payload.project_id, db)
    if not proj_name:
...
def list_submissions(
    ...
    if project_id is not None:
        ...
        q = q.filter(
            ...
            (models.AchievementSubmission.special_project == proj_name)
        )
