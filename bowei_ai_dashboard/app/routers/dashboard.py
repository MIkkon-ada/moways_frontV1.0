...
def _project_overview(
    ...
    # ── UpdateSubmissions ──────────────────────────────────
    # UpdateSubmission 无 special_project 列，只按 project_id 过滤。
    # 旧提交（project_id=NULL）在完成 migrate_project_members 后自动纳入。
    # TODO(3C-2): 迁移完成后旧提交将自动被 project_id 过滤覆盖，无需额外处理。
    ...
    # coordinator: TODO(3C-2) 只看转交给自己事项；当前保留同 owner 行为
    # member: 只看自己任务和提交
    ...
    # 5D：coordinator 裁剪——不显示 owner 待确认队列和打回明细
    if is_coord and not is_owner and not is_super:
        sub_s = {
            ...
            # TODO(5D): 补充 coordinator 专属"待统筹反馈"计数（等确认中心接口稳定后）
        }
...
def overview(
    ...
    proj_name_label: str = special_project or ""
    ...
    if effective_project_id is not None:
        ...
        proj_name = crud.get_project_name_by_id(effective_project_id, db) or proj_name_label
